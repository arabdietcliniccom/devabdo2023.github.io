...
class XmlDomParser extends AbstractDomParser
{
     ...
     * @param string $xml
     */
    protected function createDOMDocument($xml, $libXMLExtraOptions = null) : DOMDocument
    {
     ...
     * @param string $xml
     */
    public function loadXml($xml, $libXMLExtraOptions = null) : self
    {
     ...
     * @param string $filePath
     */
    public function loadXmlFile($filePath, $libXMLExtraOptions = null) : self
    {
        if (!\preg_match("/^https?:\\/\\//i", $filePath) && !\file_exists($filePath)) {
            throw new RuntimeException("File {$filePath} not found");
        }
